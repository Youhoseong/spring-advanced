...
# 프록시 내부 호출 문제
 - target에서 자기 자신의 내부 메소드 호출은 어드바이스 적용 대상이 아니다.
  
   ## 해결1. 자기자신 주입
   - Spring Bean으로 등록된 객체는 Proxy객체이기에, 이를 주입받아서 호출하면 AOP가 적용된 결과를 확인할 수 있다.
   - 단, 순환참조이기에 자제 해야한다.

   ## 해결2. 지연 조회
      - ApplicationContext를 주입받아서 거기서 Bean을 꺼낸다. => 너무 무거움.
      - ObjectProvider를 통해 스프링 컨테이너에서 조회
      - 지연 조회는 스프링 빈 생성 시점이 아니라, 실제 객체 사용 시점에 자기 자신 객체를 사용할 수 있다.

   ## 해결3. 구조적 변경
      - 내부 호출 메소드를 별도의 클래스로 분리해서 해당 클래스를 주입받자.(항상 프록시 호출)
      - 내부 호출을 물리적으로 없애버리는 것.
      - Spring에서 권장하는 방식

# JDK 동적 프록시 사용 시 문제점
 - 의존성 주입받을 시 구체 클래스 기반으로 받을 수 없음.
 - CGLIB 프록시 사용 시 이런 고민을 하지 않아도 됨

# CGLIB 프록시 사용 시 문제점
- 기본 생성자(파라미터가 한 개도 X)가 필수임. => 상속을 통해 새로운 구체 클래스를 생성하는 방식이기 때문.
- 생성자 2번 호출 문제
  - target 생성시, 프록시 생성시
  
# 스프링의 해결책
- objenesis라는 특별 라이브러리를 통해 기본 생성자 없이 객체 생성 가능하도록 함. (생성자 없이 객체 생성 가능)
- 따라서 생성자 2번 호출 문제도 자동으로 해결.
- Spring 2.0 이후부터는 CGLIB가 Default Proxy 기술.